...
def evaluate_MultiSubjectsD(data, dataset, metric='euc'):
    msg_mgr = get_msg_mgr()

    # 提取数据
    features = data['embeddings']
    labels = data['labels']
    types = data['types']    # 类型信息
    views = data['views']    # 视角信息

    # 加载探针集
    import json
    try:
        probe_sets = json.load(
            open('./datasets/MultiSubjects/MultiSubjects-D.json', 'r')
        )['PROBE_SET']
    except FileNotFoundError:
        # 使用 log_info 而不是 log_error
        msg_mgr.log_info("Probe set file not found. Please check the path.")
        return {}

    # 创建探针掩码 - 根据实际数据结构调整
    probe_mask = []
    for id, ty, view in zip(labels, types, views):
        # 构建标识符，尝试不同的格式
        identifier1 = f"{id}-{ty}-{view}"
        identifier2 = f"{id}_{ty}_{view}"
        identifier3 = f"{id}-{view}"
        identifier4 = f"{id}-{ty}"

        if identifier1 in probe_sets:
            probe_mask.append(True)
        elif identifier2 in probe_sets:
            probe_mask.append(True)
        elif identifier3 in probe_sets:
            probe_mask.append(True)
        elif identifier4 in probe_sets:
            probe_mask.append(True)
        else:
            probe_mask.append(False)
    ...
    # 检查探针集是否为空
    if np.sum(probe_mask) == 0:
        msg_mgr.log_info("No probe samples found. Please check:")
        msg_mgr.log_info(f"Probe set samples: {probe_sets[:5]}")  # 显示前5个探针样本
        sample_identifiers = [
            f'{id}-{ty}-{view}'
            for id, ty, view in list(zip(labels, types, views))[:5]
        ]
        msg_mgr.log_info(f"First few data identifiers: {sample_identifiers}")
        return {}
    ...
    # 记录数据集统计信息
    msg_mgr.log_info(f"Total samples: {len(features)}")
    msg_mgr.log_info(f"Probe samples: {len(probe_features)}")
    msg_mgr.log_info(f"Gallery samples: {len(gallery_features)}")
    msg_mgr.log_info(f"Unique probe subjects: {len(np.unique(probe_lbls))}")
    msg_mgr.log_info(f"Unique gallery subjects: {len(np.unique(gallery_lbls))}")

    # 计算距离矩阵
    msg_mgr.log_info(f"Using {metric} distance metric for evaluation.")
    dist = cuda_dist(probe_features, gallery_features, metric).cpu().numpy()
    ...
    # 记录结果
    msg_mgr.log_info(f"Evaluation results for MultiSubjectsD dataset:")
    msg_mgr.log_info(f"Rank-1: {results['scalar/test_accuracy/Rank-1']:.2f}%")
    msg_mgr.log_info(f"Rank-5: {results['scalar/test_accuracy/Rank-5']:.2f}%")
    msg_mgr.log_info(f"Rank-10: {results['scalar/test_accuracy/Rank-10']:.2f}%")
    msg_mgr.log_info(f"mAP: {results['scalar/test_accuracy/mAP']:.2f}%")
    msg_mgr.log_info(f"mINP: {results['scalar/test_accuracy/mINP']:.2f}%")

    return results

def evaluate_MultiSubjectsP(data, dataset, metric='euc'):
    msg_mgr = get_msg_mgr()
    ...
    try:
        probe_sets = json.load(
            open('./datasets/MultiSubjects/MultiSubjects-P.json', 'r')
        )['PROBE_SET']
    except FileNotFoundError:
        # 使用 log_info 而不是 log_error
        msg_mgr.log_info("Probe set file not found. Please check the path.")
    ...
    # 记录结果
    msg_mgr.log_info(f"Evaluation results for MultiSubjectsP dataset:")
    msg_mgr.log_info(f"Rank-1: {results['scalar/test_accuracy/Rank-1']:.2f}%")
    msg_mgr.log_info(f"Rank-5: {results['scalar/test_accuracy/Rank-5']:.2f}%")
    msg_mgr.log_info(f"Rank-10: {results['scalar/test_accuracy/Rank-10']:.2f}%")
    msg_mgr.log_info(f"mAP: {results['scalar/test_accuracy/mAP']:.2f}%")
...
def evaluate_MultiSubjectsS(data, dataset, metric='euc'):
    msg_mgr = get_msg_mgr()
    ...
    try:
        probe_sets = json.load(
            open('./datasets/MultiSubjects/MultiSubjects-S.json', 'r')
        )['PROBE_SET']
    except FileNotFoundError:
        # 使用 log_info 而不是 log_error
        msg_mgr.log_info("Probe set file not found. Please check the path.")
    ...
    # 记录结果
    msg_mgr.log_info(f"Evaluation results for MultiSubjectsS dataset:")
    msg_mgr.log_info(f"Rank-1: {results['scalar/test_accuracy/Rank-1']:.2f}%")
    msg_mgr.log_info(f"Rank-5: {results['scalar/test_accuracy/Rank-5']:.2f}%")
    msg_mgr.log_info(f"Rank-10: {results['scalar/test_accuracy/Rank-10']:.2f}%")
    msg_mgr.log_info(f"mAP: {results['scalar/test_accuracy/mAP']:.2f}%")

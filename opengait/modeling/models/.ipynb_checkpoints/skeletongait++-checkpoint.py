...
class CA(nn.Module):
    def __init__(self, channels, reduction=16, with_max_pool=True, choosed_part=''):
        ...
        self.gammas = torch.nn.Parameter(torch.ones(1) * 1.5)
        self.bg_weight = torch.nn.Parameter(torch.ones(1) * 0.0)
        self.non_target_weight = torch.nn.Parameter(torch.ones(1) * 0.0)
    ...
    def learnable_division(self, z, mask_resize):
        split_parts = {
            'up': [1],
            'middle': [2, 3, 4, 5, 6, 11],
            'down': [7, 8, 9, 10],
        ...
        bg_mask = (mask_long == -1)
        non_target_body_mask = (~target_mask) & (~bg_mask)
        
        target_mask = target_mask.unsqueeze(1).float()
        non_target_body_mask = non_target_body_mask.unsqueeze(1).float()
        bg_mask = bg_mask.unsqueeze(1).float()
        
        constrained_non_target_weight = 0.9 + 0.2 * torch.sigmoid(self.non_target_weight)
        constrained_bg_weight = 0.1 + 0.4 * torch.sigmoid(self.bg_weight)
        
        z_feat = (
            target_mask * z * self.gammas +
            non_target_body_mask * z * constrained_non_target_weight +
            bg_mask * z * constrained_bg_weight
        )
    ...
    def forward(self, x, y, mask_resize):
        ...
        y_feat = self.learnable_division(y_, mask_resize)
...
class PAGait(BaseModel):
    def build_network(self, model_cfg):
        ...
        # 部位注意力
        self.ca_up = CA(channels=64 * C, **model_cfg['CALayersP'], choosed_part='up')
        self.ca_middle = CA(channels=64 * C, **model_cfg['CALayersP'], choosed_part='middle')
        self.ca_down = CA(channels=64 * C, **model_cfg['CALayersP'], choosed_part='down')
    ...
    def inputs_pretreament(self, inputs):
        ...
        silhouette_parsing = inputs[0]
        new_data_list = []
        
        for sil, parsing in zip(silhouette_parsing[0], silhouette_parsing[1]):
            sil = sil[:, np.newaxis, ...]
    ...
    def forward(self, inputs):
        ...
        # 分离双输入
        parsing_input    = x[:, :1, ...]   # 轮廓图
        sil_input = x[:, 1:2, ...]  # 人体解析图
        del ipts
        ...
        # ==================== 3. 部位注意力融合（CA） ====================
        up_fused    = self.ca_up(sil2, parsing2, mask_resize)
        mid_fused   = self.ca_middle(sil2, parsing2, mask_resize)
        down_fused  = self.ca_down(sil2, parsing2, mask_resize)
        ...
        # ==================== 4. 共同特征提取（CS） ====================
        common_feat = self.cs(sil2, parsing2)

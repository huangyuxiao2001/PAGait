...
class TransferDataset(Dataset):
    def __init__(self, args, generate_heatemap_cfgs) -> None:
        ...
        assert self.dataset_name.lower() in ["sustech1k", "grew", "ccpg", "oumvlp", "ou-mvlp", "gait3d", "casiab", "casiae", "multisubjects"], f"Invalid dataset name: {self.dataset_name}"
        ...
        elif self.dataset_name.lower() == "multisubjects":
            self.all_ps_data_paths = sorted(glob(os.path.join(pose_root, "*/*/*/*.pkl")))
        else:
            self.all_ps_data_paths = sorted(glob(os.path.join(pose_root, "*/*/*.pkl")))
    
        print(f"[INFO] Dataset: {self.dataset_name}")
        print(f"[INFO] Pose root: {pose_root}")
        print(f"[INFO] Found {len(self.all_ps_data_paths)} pose files.")
        if len(self.all_ps_data_paths) == 0:
            raise ValueError(f"No pose data found in {pose_root} for dataset {self.dataset_name}")
    ...
    def __getitem__(self, index):
        ...
        with open(pose_path, "rb") as f:
            # 使用兼容模式加载pickle文件，解决版本兼容问题
            pose_data = pickle.load(f, fix_imports=True, encoding="latin1")
